...
Any `gap > 1` means events were created on the device and never arrived. **This is data
loss.** Escalate immediately.
...
([§4.5](../architecture/04-sync.md#45-sync-protocol)). A business-rule rejection means the
server is rejecting something that already physically happened, which is a **server bug**,
not a device problem. Fix the server and have the device re-push.
...
Non-zero means the Go and TypeScript pricing implementations have diverged
([§4.8](../architecture/04-sync.md#48-the-duplicated-domain-logic-problem)). Each row has a
dollar value. Find the divergent rule, add the case to the golden corpus in
...
| Pricing mismatch rate > 0 | Backend + frontend — cross-implementation defect |
